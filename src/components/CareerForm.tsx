...
export function CareerForm(props: Props) {
  ...
  const onDelete = async (id?: string) => {
    try {
      ...
      if (id) {
        await deleteCareer(id, loginUser);
      }
      deleteForm(index);
      setMessageAtom((prev) => {
        return {
  ...
  return (
    <>
      <Box>
        <Stack spacing={2}>
          ...
          <TextField
            ...
            helperText={
              errors.fieldArray?.[index]?.summary?.message as ReactNode
            }
            multiline
            rows={5}
            disabled={!watch(`fieldArray.${index}.isEditing`)}
      ...
      <SimpleDialog
        ...
        title="Confirmation"
        content="Are you sure you want to delete the career?"
        actions={[
          {
            text: "Delete",
            onClick: () => {
              onDelete(watch(`fieldArray.${index}.id`));
              setOpenDialog(false);
            },
          },
          {
